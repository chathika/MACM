...
def processData(startpos, endpos, prog, indexedKeywords, keywords_to_narratives, MainDF):
...
def GenerateTimeSeriesFromGDELT_NewsURLs_CP4(folder_name):
    ...
    for gdeltQueryFile in glob.glob(folder_name + '/*gdelt*json.gz'):
        ...
        while currentRowEnd < MainDF.shape[0]:
            ...
            paramList = [(currentRowStart + i * sizePerProc, currentRowStart + (i + 1) * sizePerProc, prog, indexedKeywords, keywords_to_narratives, MainDF) for i in range(NProcs - 1) ]
        ...
        if currentRowStart < MainDF.shape[0] and currentRowStart < currentRowEnd :
            ...
            paramList = [(currentRowStart + i * sizePerProc, currentRowStart + (i+1) * sizePerProc, prog, indexedKeywords, keywords_to_narratives, MainDF) for i in range(numProcsReq - 1)]
            print(paramList)
            if paramList == []:
