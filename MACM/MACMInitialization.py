...
def ensurePlatformUniqueness(events):
    events = events.copy()
    events.loc[events.parentID.isna(), "parentID"] = events.loc[
        events.parentID.isna(), "nodeID"
    ]
    events.loc[events.conversationID.isna(), "conversationID"] = events.loc[
        events.conversationID.isna(), "nodeID"
    ]
...
def numerifyEvents(events):
    ...
    umapping = (
        pd.Series(np.sort(list(set(userlist)))).reset_index().set_index(0).T
    )
    events["userID"] = [umapping[x][0] for x in events["userID"]]

    targetlist = (
        events.nodeID.append(events.parentID)
        .append(events.conversationID)
        .unique()
    )
    tmapping = (
        pd.Series(np.sort(list(set(targetlist)))).reset_index().set_index(0).T
    )
...
def _numerifyShocks(shocks_):
    ...
    smapping = (
        pd.Series(np.sort(list(set(shocklist)))).reset_index().set_index(0).T
    )
...
@cuda.jit()
def _cuda_resample(
    compressed_events: np.array, resampled_events: np.array
) -> None:
    ...
    for i in range(compressed_events.shape[1]):
        ...
        if time_delta == -1:
            break
        resampled_events[userID, time_delta] = (
            resampled_events[userID, time_delta] + 1
        )


def extract_endogenous_influence(
    events, verbose: bool = False
    ...
    """
    assert events.shape[0] > 0, f"There are no events {events}"
    numerified_events, u, t = _numerify_and_subset_events(events)
    H = pd.DataFrame()
    H_partial = pd.DataFrame()
    num_days = math.floor(
        (
            numerified_events.time.max() - numerified_events.time.min()
        ).total_seconds()
        / 86400
    )
    chunk_size = 100
    for day_i in tqdm(
        range(0, max(chunk_size, num_days - chunk_size), chunk_size),
        desc="Progress processing endogenous social influence",
    ):
        # Process data in chunks
        period_start = numerified_events.time.min() + dt.timedelta(days=day_i)
        period_end = numerified_events.time.min() + dt.timedelta(
            days=day_i + chunk_size
        )
        events = numerified_events[
            (numerified_events.time > period_start)
            & (numerified_events.time < period_end)
        ].copy()
        if events.shape[0] == 0:
            break
        ...
        )  # get time as hours float
        events = events[["userID", "action", "time"]].sort_values(
            ["userID", "action"]
        )
        max_events_per_user_action = (
            events.groupby(["userID", "action"]).apply(lambda x: x.time).max()
        )  # max user_action count
        events_matrix = np.zeros(
            (
                u.shape[1],
                len(list(EVENT_TO_ACTION_MAP.keys())),
                max_events_per_user_action,
            )
        )
        for action in tqdm(
            range(len(list(EVENT_TO_ACTION_MAP.keys()))),
            desc="Preparing events for cuda",
        ):
            events_this_action = events[events["action"] == action]
            max_events_by_user_this_action = (
                events_this_action.groupby("userID")
            ...
            try:
                ...
                continue
                raise Exception(
                    f"No events for action type {list(EVENT_TO_ACTION_MAP.keys())[action]}!"
                )
            for userID in range(u.shape[1]):
                events_by_user_this_action = events_this_action[
                    events_this_action["userID"] == userID
                ]
                for i, event_time in enumerate(
                    events_by_user_this_action.time
                ):
            ...
            if verbose:
                print(
                    "Resampled and matrixified "
                    + str(list(EVENT_TO_ACTION_MAP.keys())[action])
                    + " events on GPU."
                )
        ###########################################################################################################
        # Calculate entropy per action
        H_chunk = np.zeros(
            (u.shape[1], len(list(EVENT_TO_ACTION_MAP.keys()))),
            dtype=np.float32,
        )
        ...
        for action in tqdm(
            range(len(list(EVENT_TO_ACTION_MAP.keys()))),
            desc="Calculating Shannon entropy for users per action",
        ):
        ...
        H_chunk = pd.DataFrame(
            H_chunk, columns=list(EVENT_TO_ACTION_MAP.keys())
        ).fillna(0)
        ...
        H_partial_chunk = pd.DataFrame(
            H_partial_chunk, columns=list(EVENT_TO_ACTION_MAP.keys())
        ).fillna(0)
        ...
        T = pd.DataFrame(
            index=pd.MultiIndex.from_product(
                [list(range(u.shape[1])), list(range(u.shape[1]))],
                names=["userID0", "userID1"],
            )
        )
        T_partial = pd.DataFrame(
            index=pd.MultiIndex.from_product(
                [list(range(u.shape[1])), list(range(u.shape[1]))],
                names=["userID0", "userID1"],
            )
        ...
        for influencer_action in tqdm(
            range(len(list(EVENT_TO_ACTION_MAP.keys()))),
            desc="Calculating transfer entropy for users per action-action relationship",
        ):
            events_influencer_action = cuda.to_device(
                np.ascontiguousarray(events_matrix[:, influencer_action, :])
            )
            for influencee_action in range(
                len(list(EVENT_TO_ACTION_MAP.keys()))
            ):
                events_influencee_action = cuda.to_device(
                    np.ascontiguousarray(
                        events_matrix[:, influencee_action, :]
                    )
                )
                # Start cuda calculations of T
                bpg, tpb = _gpu_init_2d(
                    events_influencer_action.shape[0],
                    events_influencee_action.shape[0],
                )
                T_chunk_action_device = cuda.to_device(
                    np.zeros(
                        (
                            events_influencer_action.shape[0]
                            * events_influencee_action.shape[0]
                ...
                relationship_name = (
                    ...
                    + "To"
                    + list(EVENT_TO_ACTION_MAP.keys())[influencee_action]
                )
                T_chunk_action = pd.DataFrame(
                    T_chunk_action_device.copy_to_host().tolist(),
                ...
                T_partial_chunk_action_device = cuda.to_device(
                    np.zeros(
                        (
                            events_influencer_action.shape[0]
                            * events_influencee_action.shape[0]
                ...
                if verbose:
                    print(
                        "Transfer entropy for relationship "
                        + relationship_name
                        + " done."
                    )
        # Assimilate chunk measurements, transfer entropy can be averaged over samples
        T_chunk = (
            T_chunk.reset_index().fillna(0.0).set_index(["userID0", "userID1"])
        )
        T = T.combine(T_chunk, func=take_mean, fill_value=0)
        del T_chunk
        T_partial_chunk = (
            T_partial_chunk.reset_index()
            .fillna(0.0)
            .set_index(["userID0", "userID1"])
        )
...
def extract_exogenous_influence(
    events, shocks, verbose: bool = False
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    ...
    T = pd.DataFrame(
        index=pd.MultiIndex.from_product(
            [list(range(s.shape[1])), list(range(u.shape[1]))],
            names=["shock", "userID"],
        )
    )
    T_partial = pd.DataFrame(
    ...
    for day_i in tqdm(
        ...
        events_chunk = numerified_events[
            (numerified_events.time > period_start)
            & (numerified_events.time < period_end)
        ].copy()
        shocks_chunk = numerified_shocks[
            (numerified_shocks.time > period_start)
            & (numerified_shocks.time < period_end)
        ].copy()
        ...
        events_chunk = events_chunk[["userID", "action", "time"]].sort_values(
            ["userID", "action"]
        )
        ...
        # Cudafy shocks
        max_events_per_user_action = (
            events_chunk.groupby(["userID", "action"])
        ...
        for shockID in tqdm(
            range(s.shape[1]), desc="Preparing exogenous shocks for cuda"
        ):
            times_this_shock_occurred = shocks_chunk[[shockID]].copy()
            times_this_shock_occurred = times_this_shock_occurred[
                times_this_shock_occurred[shockID] > 0
            ].reset_index()
        ...
        H = H.combine(H_chunk, func=take_mean, fill_value=0)
        ######
        H_partial_chunk = pd.DataFrame(H_partial_chunk, columns=["H"]).fillna(
            0
        )
        ...
        T_chunk = pd.DataFrame()
        T_partial_chunk = pd.DataFrame()
        for influencee_action in tqdm(
            range(len(list(EVENT_TO_ACTION_MAP.keys()))),
            desc="Calculating transfer entropy for users per shock-action relationship",
        ):
            events_influencee_action = cuda.to_device(
        ...
        # Assimilate chunk measurements, transfer entropy can be averaged over samples
        T_chunk = (
            T_chunk.reset_index().fillna(0.0).set_index(["shockID", "userID"])
        )
        T = T.combine(T_chunk, func=take_mean, fill_value=0)
        T_partial_chunk = (
            T_partial_chunk.reset_index()
            .fillna(0.0)
            .set_index(["shockID", "userID"])
        )
        T_partial = T_partial.combine(
            T_partial_chunk, func=take_mean, fill_value=0
        )
...
def extractMessages(
    events: pd.DataFrame, network: pd.DataFrame, verbose: bool = False
) -> pd.DataFrame:
    """
    ...
    all_messages = all_messages.drop_duplicates().sort_values(
        by=["time", "action"]
    )
    if verbose:
...
def _numerify_and_subset_events(
    all_events: pd.DataFrame, verbose: bool = True
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    if verbose:
        print("Numerifying events.")
        print(
            "There are "
            + str(all_events.userID.unique().size)
            + " users. Considering all "
            + str(
                (all_events.userID.unique().size ** 2)
                * (len(list(EVENT_TO_ACTION_MAP.keys())) ** 2)
            )
            + " possible relationships"
        )
    assert (
        all_events.userID.unique().size > 0
    ), f"No users with minimal required monthly event count {ACTIVITY_THRESHOLD}"
    users_to_consider = all_events.groupby(["userID"]).apply(
        lambda x: x.set_index("time").resample("M").count().iloc[:, 0].mean()
        > ACTIVITY_THRESHOLD[x.platform.iloc[0].lower()]
    )
    users_to_consider = users_to_consider[
        users_to_consider == True
    ].index.unique()
...
def main():
    ...
    parser.add_argument(
        "event_file",
        help="event file to be used to infer endo/exo-genous influence.",
    )
    parser.add_argument(
        "shocks_file",
        help="event file to be used to infer endo/exo-genous influence.",
    ...
    events = events[
        (
            events.time
            > dt.datetime.strptime(args.time_min, "%Y-%m-%dT%H:%M:%SZ")
        )
        & (
            events.time
            < dt.datetime.strptime(args.time_max, "%Y-%m-%dT%H:%M:%SZ")
        )
    ]

    out_dir = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "init_data"
    )

    # Calculate endogenous social influence
    H, H_partial, T, T_partial = extract_endogenous_influence(events.copy())
    H.to_csv(
        os.path.join(out_dir, "MACM_Init_Endogenous_Entropy.csv"), index=False
    )
    H_partial.to_csv(
        os.path.join(out_dir, "MACM_Init_Endogenous_Partial_Entropy.csv"),
        index=False,
    )
    T.to_csv(
        os.path.join(out_dir, "MACM_Init_Endogenous_Transfer_Entropy.csv"),
        index=False,
    )
    T_partial.to_csv(
        os.path.join(
            out_dir, "MACM_Init_Endogenous_Partial_Transfer_Entropy.csv"
        ),
        index=False,
    )
    ...
    messages.to_csv(
        os.path.join(out_dir, "MACM_Init_Messages.csv"), index=False
    )
    del T
    ...
    H.to_csv(
        os.path.join(out_dir, "MACM_Init_Exogenous_Entropy.csv"), index=False
    )
    H_partial.to_csv(
        os.path.join(out_dir, "MACM_Init_Exogenous_Partial_Entropy.csv"),
        index=False,
    )
    T.to_csv(
        os.path.join(out_dir, "MACM_Init_Exogenous_Transfer_Entropy.csv"),
        index=False,
    )
    T_partial.to_csv(
        os.path.join(
            out_dir, "MACM_Init_Exogenous_Partial_Transfer_Entropy.csv"
        ),
        index=False,
    )
