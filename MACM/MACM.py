...
class MACM:
    ...
    def __init__(self, START_TIME, TICKS_TO_SIMULATE, MAX_MEMORY_DEPTH, MEMORY_DEPTH_FACTOR, QUIET_MODE=False, DEVICE_ID=0, DUMP_AGENT_MEMORY=False, ENABLE_CONTENT_MUTATION=False, ENABLE_MODEL_P=True, ENABLE_MODEL_I=True):
        ...
        self.START_TIME = dt.datetime.strptime(
            str(START_TIME), "%Y-%m-%dT%H:%M:%S%fZ")
        ...
        # first cols of .csv are userID,action,nodeID,parentID,conversationID,rootID,informationIDs
        self.MESSAGE_ITEM_COUNT = 5 + self.MAX_NUM_INFORMATION_IDS_PER_EVENT
        ...
        self.DATA_FOLDER_PATH = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "init_data"))
        self.OUTPUT_FOLDER_PATH = os.path.abspath(
    ...
    def propagate(self, edges, outgoing_messages, received_information):
        ...
        for message_idx in prange(len(outgoing_messages)):
            ...
            receivers_new_memory = np.full(
                (received_information.shape[1], self.MESSAGE_ITEM_COUNT), -1, dtype=np.float64)
            for receiver in receivers:
                receivers_new_memory[1:] = new_received_information[int(
                    receiver), :-1]
    ...
    def initialize_model(self):
        ...
        self.Data_Endo = {}
        self.Data_Endo["TE"] = pd.read_csv(glob.glob(os.path.join(
            self.DATA_FOLDER_PATH, '*Endogenous_Partial_Transfer_Entropy*.csv'))[0])
        self.Data_Endo["E"] = pd.read_csv(glob.glob(os.path.join(
            self.DATA_FOLDER_PATH, '*Endogenous_Partial_Entropy*.csv'))[0])
        # Create Entropy dataframe and fill with data
        df = self.Data_Endo["E"].rename(
            columns={"userID": "userID1"}).set_index("userID1")
        a = pd.DataFrame()
        ...
        self.Data_Exo = {}
        self.Data_Exo["TE"] = pd.read_csv(glob.glob(os.path.join(
            self.DATA_FOLDER_PATH, '*Exogenous_Partial_Transfer_Entropy*.csv'))[0])
        self.Data_Exo["E"] = pd.read_csv(glob.glob(os.path.join(
            self.DATA_FOLDER_PATH, '*Endogenous_Partial_Entropy*.csv'))[0])
        ...
        joined_endo = self.Data_Endo["TE"].set_index(["userID0", "userID1"]).sort_index().join(self.Data_Endo["E"].set_index(
            ["userID0", "userID1"]).sort_index(), how="outer", lsuffix="TE", rsuffix="E").fillna(0).sort_index().reset_index()
        joined_endo = joined_endo[joined_endo.userID0 != joined_endo.userID1]
        joined_exo = self.Data_Exo["TE"].set_index(["shockID", "userID"]).sort_index().join(self.Data_Exo["E"].set_index(
            ["shockID", "userID"]).sort_index(), how="outer", lsuffix="TE", rsuffix="E").fillna(0).sort_index().reset_index()
        self.MACM_print("Numerifying user and shock ids")
        userlist = joined_endo.reset_index().userID1.append(joined_endo.reset_index(
        ).userID0).append(joined_exo.reset_index().userID).unique().tolist()
        joined_exo = joined_exo[joined_exo["userID"].isin(userlist)]
        # Numerify the user ids
        self.umapping = pd.Series(
            np.sort(list(set(userlist)))).reset_index().set_index(0).T
        shocklist = joined_exo.reset_index().shockID.unique()
        self.smapping = pd.Series(
            np.sort(list(set(shocklist)))).reset_index().set_index(0).T
        joined_endo["userID0"] = [self.umapping[x][0]
                                  for x in joined_endo["userID0"]]
        joined_endo["userID1"] = [self.umapping[x][0]
                                  for x in joined_endo["userID1"]]
        joined_exo["shockID"] = [self.smapping[x][0]
                                 for x in joined_exo["shockID"]]
        joined_exo["userID"] = [self.umapping[x][0]
                                for x in joined_exo["userID"]]
        # reindex
        joined_endo = joined_endo.sort_values(
            ["userID0", "userID1"]).set_index(["userID0", "userID1"])
        joined_exo = joined_exo.sort_values(
            ["shockID", "userID"]).set_index(["shockID", "userID"])
        # Convert to Matrices
        ...
        non_zero_joined_endo = joined_endo[joined_endo[relationshipsTE].sum(
            axis=1) > 0]
        # Calculate TE / E values by dividing TE by the corresponding E. NaN values are replaced by 0.
        df = (non_zero_joined_endo[relationshipsTE].fillna(
            0) / (non_zero_joined_endo[relationshipsE].fillna(0).values)).fillna(0)
        self.Data_Endo["q"] = np.empty(
            (df.shape[0], len(Events.getEventTypes()), len(Events.getEventTypes())))
        for ida in range(len(Events.getEventTypes())):
            for idb in range(len(Events.getEventTypes())):
                ...
                self.Data_Endo["q"][:, ida, idb] = df.iloc[:,
                                                           (len(Events.getEventTypes()) * ida) + idb].values
        self.Data_Endo["edges"] = df.reset_index()[
            ["userID0", "userID1"]].values
        ...
        for idx in range(self.Data_Endo["edges"].shape[0]):
            if self.Data_Endo["edges"][idx, 0] >= u0:
                self.Data_Endo["Influencer_Index"][self.Data_Endo["edges"]
                                                   [idx, 0]] = idx
                u0 = self.Data_Endo["edges"][idx, 0] + 1
        #######
        self.MACM_print("Setting Up Exogenous Influence")
        self.MACM_print(joined_exo.head())
        non_zero_joined_exo = joined_exo[joined_exo[[
            e + "TE" for e in Events.getEventTypes()]].sum(axis=1) > 0]
        df = (non_zero_joined_exo[[e + "TE" for e in Events.getEventTypes()]].fillna(0) / (
            non_zero_joined_exo[[e + "E" for e in Events.getEventTypes()]].fillna(0).abs().values)).fillna(0)
        ...
        for idx in range(self.Data_Exo["edges"].shape[0]):
            if self.Data_Exo["edges"][idx, 0] >= s0:
                self.Data_Exo["Shock_Index"][self.Data_Exo["edges"]
                                             [idx, 0]] = idx
        ...
        df = joined_endo[relationshipsTE].reset_index().groupby("userID1").apply(lambda x: x.sum(
        )).drop(["userID0", "userID1"], axis=1).rename(index={"userID1": "userID"}).copy()
        ...
        for eventType in Events.getEventTypes():
            ...
            df1 = df1.join(df[influencingCols].sum(axis=1).rename(
                eventType + "EndoInf"), how="outer")
        # Same for exo
        # Find total info flow in to users by relationship to shocks
        df2 = joined_exo[[e + "TE" for e in Events.getEventTypes()]].reset_index().groupby(
            "userID").apply(lambda x: x.sum()).drop(["shockID", "userID"], axis=1).copy()
        ...
        df = joined_endo[relationshipsE].reset_index().groupby("userID0").apply(lambda x: x.mean(
        )).drop(["userID0", "userID1"], axis=1).rename(index={"userID1": "userID"}).copy()
        for eventType in Events.getEventTypes():
            influencingCols = []
            for col in df.columns:
                if ("To" + eventType) in col:
                    influencingCols.append(col)
            df1 = df1.join(df[influencingCols].sum(
                axis=1).rename(eventType + "IntInf"), how="outer")
        ...
        for eventType in Events.getEventTypes():
            ...
            df = df.join(((df1[(eventType + "IntInf")] - df1[(eventType + "EndoInf")] - df1[(
                eventType + "ExoInf")]) / df1[(eventType + "IntInf")].values).rename(eventType), how="outer")
        prob = pd.read_csv(glob.glob(os.path.join(
            self.DATA_FOLDER_PATH, '*Endogenous_Hourly_Activity_Probability*.csv'))[0])
        ...
        Data_Msg = pd.read_csv(glob.glob(os.path.join(self.DATA_FOLDER_PATH, '*Messages*.csv'))[0], parse_dates=[
                               "time"]).set_index("time").tz_localize(None).reset_index().sort_values("time", ascending=True)
        ...
        targetlist = Data_Msg["nodeID"].append(
            Data_Msg["parentID"]).append(Data_Msg["conversationID"])
        self.tmapping = np.sort(list(set(targetlist)))
        Data_Msg["nodeID"] = Data_Msg.apply(
            lambda x: np.nonzero(self.tmapping == x.nodeID)[0][0], axis=1)
        Data_Msg["parentID"] = Data_Msg.apply(
            lambda x: np.nonzero(self.tmapping == x.parentID)[0][0], axis=1)
        Data_Msg["conversationID"] = Data_Msg.apply(
            lambda x: np.nonzero(self.tmapping == x.conversationID)[0][0], axis=1)
        # construct information ID mapping and numerify informationIDs
        informationIDslist = set()
        if self.ENABLE_CONTENT_MUTATION:
            df_contentIDprobs = pd.read_csv(glob.glob(os.path.join(
                self.DATA_FOLDER_PATH, "*Endogenous_ContentIDProbDists*"))[0])
            informationIDslist = set(df_contentIDprobs.parentNarrative)
        Data_Msg.informationIDs.apply(lambda x: informationIDslist.update(
            eval(x) if type(eval(x)) == list else [eval(x)]))
        informationIDslist = list(informationIDslist)
        self.imapping = pd.Series(
            np.sort(informationIDslist)).reset_index().set_index(0).T
        ...
        for x in Data_Msg["informationIDs"]:
            if type(eval(x)) == list:
                msg_informationids.append(
                    [self.imapping[str(y)][0] for y in eval(x)])
                if len(msg_informationids[-1]) > self.MAX_NUM_INFORMATION_IDS_PER_EVENT:
                    self.MAX_NUM_INFORMATION_IDS_PER_EVENT = len(
                        msg_informationids[-1])
            else:
        ...
        # userID,action,nodeID,parentID,conversationID/rootID,informationIDs
        self.MESSAGE_ITEM_COUNT = 5 + self.MAX_NUM_INFORMATION_IDS_PER_EVENT
        ...
        self.Received_Information = np.full(
            (self.umapping.size, self.RECEIVED_INFORMATION_LIMIT, self.MESSAGE_ITEM_COUNT), -1, dtype=np.float64)
        ...
        for idx, msg in Data_Msg.iterrows():
            ...
            msgdata[:5] = [msg.userID, int(Events.getEventTypeIdx(msg.action)), int(
                msg.nodeID), int(msg.parentID), int(msg.conversationID)]
        ...
        self.Received_Information = self.propagate(
            self.Data_Endo["edges"], MessagesToPropagate, self.Received_Information)
        ...
        shocks = pd.read_csv(glob.glob(os.path.join(
            self.DATA_FOLDER_PATH, '*shocks*.csv'))[0], parse_dates=["time"])
        shocks = shocks[shocks.time > self.START_TIME].sort_values(
            "time", ascending=False).set_index("time").resample("60S").sum().fillna(0)
        self.Data_Exo["shocks"] = np.full(
            (shocks.shape[0], self.smapping.size), -1, dtype=np.int32)
        for idx, shock in enumerate(self.smapping):
            if shock in shocks.columns:
                self.Data_Exo["shocks"][:, idx] = shocks[shock].values
            else:
                self.Data_Exo["shocks"][:, idx] = np.full(
                    shocks.shape[0], 0, dtype=np.int32)
        # Read contentID conditional probability values
        self.MACM_print("\n\tNumOfUsers: {}\n\tNumContentIDs(InfoIDs): {}".format(
            self.umapping.size, self.NUM_UNIQUE_INFO_IDS))
        # Default content mask set to identity:
        self.Data_Endo["content_mutation_mask"] = np.empty(
            (self.umapping.size, self.NUM_UNIQUE_INFO_IDS, self.NUM_UNIQUE_INFO_IDS), dtype=float)
        for u in range(self.umapping.size):
            self.Data_Endo["content_mutation_mask"][u] = np.identity(
                self.NUM_UNIQUE_INFO_IDS, dtype=float)
        if self.ENABLE_CONTENT_MUTATION:
            ...
            if not set(informationIDslist).issubset(df_contentIDprobs.columns):
                print(
                    "ERROR: The informationIDs required are not a subset of the available data.")
            # Filter the informationIDs and order them to match the imapping order
            df_contentIDprobs = df_contentIDprobs.reindex(informationIDslist)[
                informationIDslist]
            for u in range(self.umapping.size):
            ...
            if len(glob.glob(os.path.join(self.DATA_FOLDER_PATH, "*Endogenous_UserBasedContentIDProbDists*"))) > 0:
                df_contentIDUserprobs = pd.read_csv(glob.glob(os.path.join(
                    self.DATA_FOLDER_PATH, "*Endogenous_UserBasedContentIDProbDists*"))[0])
                for userParentNar in df_contentIDUserprobs.groupby(['userID', 'parentInfoID']):
                    self.Data_Endo["content_mutation_mask"][self.umapping[userParentNar[0][0]][0],
                                                            self.imapping[userParentNar[0][1]][0]] = np.zeros(self.imapping.size, dtype=float)
                for i, row in df_contentIDUserprobs.iterrows():
                    self.Data_Endo["content_mutation_mask"][self.umapping[row['userID']][0],
                                                            self.imapping[row['parentInfoID']][0], self.imapping[row['childInfoID']][0]] = row['probVals']
    ...
    def run(self):
        ...
        Endo_Inf_Idx_global_mem = cuda.to_device(
            self.Data_Endo["Influencer_Index"])
        ...
        shocks_tmp_counter = cuda.to_device(
            np.full(self.RECEIVED_INFORMATION_LIMIT, -1, dtype=np.float64))
        # Internal
        ...
        outgoing_messages = np.full(
            (self.umapping.size, self.RECEIVED_INFORMATION_LIMIT, self.MESSAGE_ITEM_COUNT), -1, dtype=np.float64)
        om_global_mem = cuda.to_device(outgoing_messages)
        actionable_information = np.full(
            (self.Received_Information.shape[0], self.MAX_MEMORY_DEPTH, self.MESSAGE_ITEM_COUNT), -1, dtype=np.float64)
        ai_global_mem = cuda.to_device(actionable_information)
        current_memory_depths = np.full(
            self.Received_Information.shape[0], self.MAX_MEMORY_DEPTH, dtype=np.float64)
        cmd_global_mem = cuda.to_device(current_memory_depths)
        # narrative possible replies
        content_mutation_mask_global_mem = cuda.to_device(
            self.Data_Endo["content_mutation_mask"])
        # Configure the blocks
        TPB = 16
        threadsperblock = TPB
        blockspergrid = int(
            math.ceil(self.Received_Information.shape[0] / threadsperblock))
        rng_states = create_xoroshiro128p_states(
            blockspergrid * threadsperblock, seed=1)
        ...
        while s < ticks:
            ...
            exo_shocks_global_mem = cuda.to_device(
                self.Data_Exo["shocks"][s].copy())
            recompute_memory_gpu[blockspergrid, threadsperblock](rng_states, ri_global_mem, ai_global_mem, cmd_global_mem, exo_shocks_global_mem,
                                                                 shocks_tmp_counter, self.MAX_MEMORY_DEPTH, self.MEMORY_DEPTH_FACTOR, self.RECEIVED_INFORMATION_LIMIT, self.MESSAGE_ITEM_COUNT)
            if self.DUMP_AGENT_MEMORY:
            ...
            p_by_action_global_mem = cuda.to_device(
                np.full((self.umapping.size, Events.et), 1, dtype=np.float64))
            uniq_global_mem = cuda.to_device(np.array(
                [int(s << math.ceil(math.log(self.Received_Information.shape[0], 2)))]))
        ...
        print("Took : " + str(diag_time_stop-diag_time_start) + " seconds to run " +
              str((self.START_TIME - self.START_TIME + dt.timedelta(hours=s)).days) + " days")

        all_events = pd.DataFrame(all_events, columns=[
                                  "time", "userID", "action", "nodeID", "parentID", "conversationID", "informationIDs"])
        all_events["time"] = all_events.iloc[:, 0].apply(
            lambda x: self.START_TIME + dt.timedelta(hours=x))
        all_events["userID"] = [
            self.umapping.columns[int(x)] for x in all_events.iloc[:, 1]]
        all_events["action"] = all_events.iloc[:, 2].apply(
            lambda x: Events.getEventTypes()[int(x)])
        #all_events["nodeID"]=all_events.iloc[:,3].apply(lambda x: self.tmapping[int(x)] if int(x) + 1 < len(self.tmapping) else x)
        all_events["parentID"] = all_events.iloc[:, 4].apply(lambda x: self.tmapping[int(
            x)] if (int(x) >= 0) and (int(x) + 1 < len(self.tmapping)) else x)
        all_events["conversationID"] = all_events.iloc[:, 5].apply(lambda x: self.tmapping[int(
            x)] if (int(x) >= 0) and (int(x) + 1 < len(self.tmapping)) else x)
        all_events["informationIDs"] = all_events.iloc[:, 6].apply(
            lambda x: [self.imapping.columns[int(id)] if id >= 0 else "-1.0" for id in x])

        identifier = str(dt.datetime.now())
        model_type_string = 'Q{}{}-{}'.format(('P' if self.ENABLE_MODEL_P else 'X'), (
            'I' if self.ENABLE_MODEL_I else 'X'), ('CA' if self.ENABLE_CONTENT_MUTATION else 'XX'))
        file_name = os.path.join(self.OUTPUT_FOLDER_PATH, "MACM_{0}_MMD{1}_Alpha{2}_{3}.csv".format(
            model_type_string, self.MAX_MEMORY_DEPTH, self.MEMORY_DEPTH_FACTOR, identifier))
        all_events.to_csv(file_name, index=False)
        print('Output written to :{}'.format(file_name))

        if self.DUMP_AGENT_MEMORY:
            recI_t = pd.DataFrame(recI_t, columns=[
                                  "time", "influenceeID", "influencerID", "action", "nodeID", "parentID", "conversationID", "informationIDs"])
            print(recI_t)
            recI_t["time"] = recI_t.iloc[:, 0].apply(
                lambda x: self.START_TIME + dt.timedelta(hours=x))
            recI_t["influenceeID"] = [self.umapping.columns[int(
                x)] if x > -1 else x for x in recI_t.iloc[:, 1]]
            recI_t["influencerID"] = [self.umapping.columns[int(
                x)] if x > -1 else x for x in recI_t.iloc[:, 2]]
            recI_t["action"] = recI_t.iloc[:, 3].apply(
                lambda x: Events.getEventTypes()[int(x)] if x > -1 else x)
            ...
            actI_t = pd.DataFrame(actI_t, columns=[
                                  "time", "influenceeID", "influencerID", "action", "nodeID", "parentID", "conversationID", "informationIDs"])
            print(actI_t)
            actI_t["time"] = actI_t.iloc[:, 0].apply(
                lambda x: self.START_TIME + dt.timedelta(hours=x))
            actI_t["influenceeID"] = [self.umapping.columns[int(
                x)] if x > -1 else x for x in actI_t.iloc[:, 1]]
            actI_t["influencerID"] = [self.umapping.columns[int(
                x)] if x > -1 else x for x in actI_t.iloc[:, 2]]
            actI_t["action"] = actI_t.iloc[:, 3].apply(
                lambda x: Events.getEventTypes()[int(x)] if x > -1 else x)
            ...
            cmd_t["time"] = cmd_t.iloc[:, 0].apply(
                lambda x: self.START_TIME + dt.timedelta(hours=x))
            cmd_t["userID"] = [self.umapping.columns[int(x)] if int(
                x) > -1 else int(x) for x in cmd_t.iloc[:, 1]]

        if self.DUMP_AGENT_MEMORY:
            file_name = os.path.join(self.OUTPUT_FOLDER_PATH, "MACM_RI_MMD{0}_Alpha{1}_{2}.csv".format(
                self.MAX_MEMORY_DEPTH, self.MEMORY_DEPTH_FACTOR, identifier))
            recI_t.to_csv(file_name, index=False)
            file_name = os.path.join(self.OUTPUT_FOLDER_PATH, "MACM_AI_MMD{0}_Alpha{1}_{2}.csv".format(
                self.MAX_MEMORY_DEPTH, self.MEMORY_DEPTH_FACTOR, identifier))
            actI_t.to_csv(file_name, index=False)
            file_name = os.path.join(self.OUTPUT_FOLDER_PATH, "MACM_CMD_MMD{0}_Alpha{1}_{2}.csv".format(
                self.MAX_MEMORY_DEPTH, self.MEMORY_DEPTH_FACTOR, identifier))
...
def step(rng_states, inf_idx_Qs, edges_Qs, Qs, inf_idx_Ps, edges_Ps, Ps, p_by_action, messages, outgoing_messages, shocks, Is, uniq, content_mutation_mask,
    ...
    for outgoing_message_idx in range(outgoing_messages.shape[1]):
        for message_item in range(outgoing_messages.shape[2]):
            outgoing_messages[influencee_id,
                              outgoing_message_idx, message_item] = -1
    outgoing_message_idx = 0
    num_ai = 0
    # Iterate over the messages in actionable information.
    for message_idx in range(messages.shape[1]):
        most_recent_influencer_id = int(
            messages[influencee_id, message_idx, 0])
        most_recent_influencer_action = int(
            messages[influencee_id, message_idx, 1])
        ...
        for message_jdx in range(messages.shape[1]):
            most_recent_influencer_id_shock = int(
                messages[influencee_id, message_jdx, 0])
            most_recent_influencer_action_shock = int(
                messages[influencee_id, message_jdx, 1])
            if most_recent_influencer_id_shock >= 0 and most_recent_influencer_action == -1:
                ...
                if is_message_about_a_shock:
                    ...
                    for possible_influencee_action_to_shock in range(Events.et):
                        p_by_action[influencee_id, possible_influencee_action_to_shock] = p_by_action[influencee_id,
                                                                                                      possible_influencee_action_to_shock] * (1 - Ps[edge_idx_P, possible_influencee_action_to_shock])
        for possible_influencee_action_to_shock in range(Events.et):
            p_by_action[influencee_id, possible_influencee_action_to_shock] = 1 - \
                p_by_action[influencee_id, possible_influencee_action_to_shock]
        ####Done with shocks#####
        if most_recent_influencer_id >= 0 and most_recent_influencer_action >= 0:
            ...
            while possible_influencee_action < Events.et:
                message_qt = Qs[edge_idx_Q, most_recent_influencer_action,
                                possible_influencee_action]
                message_pt = p_by_action[influencee_id,
                                         possible_influencee_action] if ENABLE_MODEL_P else 0
                ...
                if rnd < prob:
                    if influencee_action_prob < prob:
                        influencee_action_taken = int(
                            possible_influencee_action)
                        influencee_action_prob = prob
                possible_influencee_action = int(
                    possible_influencee_action + 1)
            if influencee_action_prob > 0 and influencee_action_taken >= 0:
                # construct outgoing message
                outgoing_messages[influencee_id,
                                  outgoing_message_idx, 0] = influencee_id
                outgoing_messages[influencee_id,
                                  outgoing_message_idx, 1] = influencee_action_taken
                outgoing_messages[influencee_id, outgoing_message_idx, 2] = int(
                    uniq[0] + influencee_id) + (event_number / RECEIVED_INFORMATION_LIMIT)
                event_number += 1
                if np.int32(influencee_action_taken) != np.int32(Events.creation_idx):
                    outgoing_messages[influencee_id, outgoing_message_idx, 3] = int(
                        messages[influencee_id, message_idx, 2])  # parentID
                    outgoing_messages[influencee_id, outgoing_message_idx, 4] = int(
                        messages[influencee_id, message_idx, 4])
                else:
                    outgoing_messages[influencee_id, outgoing_message_idx, 4] = int(
                        outgoing_messages[influencee_id, outgoing_message_idx, 2])  # conversationID ...is nodeID if action is creation
                    outgoing_messages[influencee_id, outgoing_message_idx, 3] = int(
                        outgoing_messages[influencee_id, outgoing_message_idx, 2])  # parentID ...is nodeID if action is creation
                # setting up the list of reply content
                for message_item_idx in range(5, MESSAGE_ITEM_COUNT):
                    outgoing_messages[influencee_id, outgoing_message_idx,
                                      message_item_idx] = messages[influencee_id, message_idx, message_item_idx]
                    if messages[influencee_id, message_idx, message_item_idx] >= 0 and messages[influencee_id, message_idx, message_item_idx] <= NUM_UNIQUE_INFO_IDS:
                        parent_content_id = int(
                            messages[influencee_id, message_idx, message_item_idx])
                        rnd = xoroshiro128p_uniform_float64(
                            rng_states, influencee_id)
                        prob = 0.0
                        for rep_content_id in range(NUM_UNIQUE_INFO_IDS):
                            prob += content_mutation_mask[influencee_id,
                                                          parent_content_id, rep_content_id]
                            if rnd < prob:
                                outgoing_messages[influencee_id, outgoing_message_idx,
                                                  message_item_idx] = rep_content_id
                                break
                if outgoing_messages[influencee_id, outgoing_message_idx, 5] < 0:
                    outgoing_messages[influencee_id, outgoing_message_idx, 5] = int(
                        xoroshiro128p_uniform_float64(rng_states, influencee_id) * NUM_UNIQUE_INFO_IDS)
                # ---
                ...
                while ai_idx < MAX_MEMORY_DEPTH - 1:
                    for message_item in range(MESSAGE_ITEM_COUNT):
                        # shift message
                        messages[influencee_id, ai_idx, int(
                            message_item)] = messages[influencee_id, ai_idx+1, int(message_item)]
                    ai_idx = ai_idx + 1
                # Finally, push in the empty message
                for message_item in range(MESSAGE_ITEM_COUNT):
                    messages[influencee_id, MAX_MEMORY_DEPTH -
                             1, int(message_item)] = -1
    if ENABLE_MODEL_I:
        for possible_action, I in enumerate(Is[influencee_id, :]):
            ...
            if rnd < I:
                ...
                outgoing_messages[influencee_id,
                                  outgoing_message_idx, 0] = influencee_id
                outgoing_messages[influencee_id,
                                  outgoing_message_idx, 1] = possible_action
                outgoing_messages[influencee_id, outgoing_message_idx, 2] = int(
                    uniq[0] + influencee_id) + (event_number / RECEIVED_INFORMATION_LIMIT)
                event_number += 1
                # construct 3,4,5,.. indexes as for a creation event for now
                outgoing_messages[influencee_id, outgoing_message_idx, 3] = int(
                    outgoing_messages[influencee_id, outgoing_message_idx, 2])
                outgoing_messages[influencee_id, outgoing_message_idx, 4] = int(
                    outgoing_messages[influencee_id, outgoing_message_idx, 2])
                outgoing_messages[influencee_id, outgoing_message_idx, 5] = int(
                    xoroshiro128p_uniform_float64(rng_states, influencee_id) * NUM_UNIQUE_INFO_IDS)
                # check if there are no messages for anyone
                if messages.shape[1] > 0:
                    if possible_action != np.int32(Events.creation_idx):
                        ...
                        if valid_msg_count > 0:
                            selected_message_id = int(xoroshiro128p_uniform_float64(
                                rng_states, influencee_id) * valid_msg_count)
                            msg_id = 0
                            while msg_id < (len(messages[influencee_id])):
                                ...
                                msg_id += 1
                            outgoing_messages[influencee_id, outgoing_message_idx, 3] = int(
                                messages[influencee_id, msg_id, 2])  # parentID is nodeID of parent
                            outgoing_messages[influencee_id, outgoing_message_idx, 4] = int(
                                messages[influencee_id, msg_id, 4])  # conversationID of converstaionID of parent
                            outgoing_messages[influencee_id, outgoing_message_idx, 5] = int(
                                messages[influencee_id, msg_id, 5])
                        else:
                            # no messages available! treat as creation!
                            outgoing_messages[influencee_id, outgoing_message_idx, 1] = np.int32(
                                Events.creation_idx)
                else:
                    # no messages available! treat as creation!
                    outgoing_messages[influencee_id, outgoing_message_idx, 1] = np.int32(
    ...
    while outgoing_messages[int(sender_id), outgoing_message_idx, 0] > -1:
        ...
        while (edge_idx >= 0) and (edge_idx < edges.shape[0]) and (edges[edge_idx, 0] == sender_id):
            ...
            if (receiver_id != sender_id) and (receiver_id != outgoing_messages[int(sender_id), outgoing_message_idx, 0]) and (outgoing_messages[int(sender_id), outgoing_message_idx, 0] >= 0) and (receiver_id >= 0):
                ...
                while i > 0:
                    for message_item in range(MESSAGE_ITEM_COUNT):
                        received_information[receiver_id, i, int(
                            message_item)] = received_information[receiver_id, i-1, int(message_item)]
                    i = i - 1
                # Finally, push in the new message
                for message_item in range(MESSAGE_ITEM_COUNT):
                    received_information[receiver_id, 0, int(message_item)] = outgoing_messages[int(
                        sender_id), outgoing_message_idx, int(message_item)]
            edge_idx = edge_idx + 1
...
@cuda.jit()
def recompute_memory_gpu(rng_states, received_information, actionable_information, cmd, shocks, shuffled_ri_idxs, MAX_MEMORY_DEPTH, MEMORY_DEPTH_FACTOR, RECEIVED_INFORMATION_LIMIT, MESSAGE_ITEM_COUNT):
    influencee_id = int(cuda.grid(1))
    ...
    for shock_ID, shock in enumerate(shocks):
        if shock > 0 and current_ri_length < RECEIVED_INFORMATION_LIMIT:
            ...
            received_information[influencee_id, int(
                current_ri_length), 0] = shock_ID
    ...
    for i in range(current_ri_length, 0, -1):
        rnd_idx = int(xoroshiro128p_uniform_float64(
            rng_states, influencee_id) * i)
    ...
    current_overload_influencee = current_ai_length + \
        current_ri_length - MAX_MEMORY_DEPTH
    ...
    current_memory_depth_influencee = current_memory_depth_influencee - \
        (current_overload_influencee ** MEMORY_DEPTH_FACTOR)
        ...
        while ai_idx > 0:
            for message_item in range(MESSAGE_ITEM_COUNT):
                ...
                if ai_idx > current_memory_depth_influencee - 1:
                    actionable_information[influencee_id,
                                           ai_idx, int(message_item)] = -1
                else:
                    # else shift message
                    actionable_information[influencee_id, ai_idx, int(
                        message_item)] = actionable_information[influencee_id, ai_idx-1, int(message_item)]
            ai_idx = ai_idx - 1
        # Finally, push in the new message
        for message_item in range(MESSAGE_ITEM_COUNT):
            actionable_information[influencee_id, 0, int(message_item)] = received_information[influencee_id, int(
                shuffled_ri_idxs[ri_idx]), int(message_item)]
        ri_idx = ri_idx - 1
