...
def getOutliers(crypto_data, name, field, delta, freq_amp_threshold=0.1):    
    ...
    try:
        ...
        ax1.set_xlim([datetime(2015,1,1),datetime(2018,1,1)])
        ...
        plt.savefig(os.path.join(name +"_" + field + "_Difference_with_Outliers.png"))
    except:
...
def extractOutliers():
    ...
    for data in all_exogenous_data.keys() :    
        print("Extracting exogenous shocks from: " + str(data))
        if "Crypto_Price" in data:
            name = data[len("Crypto_Price")+1:-4]
            crypto_data = all_exogenous_data[data]  
            crypto_volumeto = crypto_data["volumeto"].diff()      
            outlier_volumeto_df = getOutliers(crypto_volumeto,name,"volumeto", 3)
            outlier_volumeto_df.columns = [data[13:-5] + "_" + "volumeto"]

            crypto_open = crypto_data["open"].diff()      
            outlier_open_df = getOutliers(crypto_open,name,"open", 3)
            outlier_open_df.columns = [data[13:-5] + "_" + "open"]
        ...
        elif "hackernews" in data:
            hackernews_data = all_exogenous_data[data]
            ...
            name = data[1:]
            nvd_data = all_exogenous_data[data]
    ...
    all_exogenous_shocks.to_csv("all_exogenous_shocks_scen{}.csv".format(args.scenario))
    # all_exogenous_shocks.replace(True,1).replace(False,0).fillna(0).to_csv("all_exogenous_shocks_scen{}.csv".format(args.scenario), index=False)
...
def readOutliersAndSubsetForSimulation(_startDate,_stopDate):
    ...
    exogenous_shocks.to_csv(DIRECTORY +"/Simulation_Ready_Exogenous_Shocks.csv", index=True, date_format = "%Y-%m-%dT%H:%M:%SZ")
